...
def retrain_train(sess, env, X_data, y_data, X_valid=None, y_valid=None, epochs=1,
    ...
    if not hasattr(env, 'saver'):
        ...
        return
    print('\nLoading saved model')
    env.saver.restore(sess, '/home/shayan/CIFAR/tensorflow-adversarial-master/example/deepfool_model/pix2pix_adv/iter3/{}'.format(name))
    ...
    if hasattr(env, 'saver'):
        print('\n Saving model')
        env.saver.save(sess, '/home/shayan/CIFAR/tensorflow-adversarial-master/example/deepfool_model/pix2pix_adv/iter4/{}'.format(name))
...
len_list = []
for img in glob.glob('/home/shayan/Proposal_Pix2Pix/Data/Training/DeepFool/Iteration_4/Training_Data/Epoch_10/outputs/' + '*.png'):
...
for i in range(0,6773):
    ...
    if deepfool_acc <= 0.20:
        fig = plt.figure()
        plt.imshow(xadvs_0)
        plt.imsave('/home/shayan/Proposal_Pix2Pix/Data/Training/DeepFool/Iteration_4/Retraining_Results_Pix2Pix_Adv/Epoch_10/Fooling_Data/xadvs_' + str(i) + '_deepfool_' + '_' + str(deepfool_acc) + '_[' + str((int(y0[0])*0+int(y0[1])*1)) + '].png',xadvs_0)
        plt.close(fig)
    else:
        fig = plt.figure()
        plt.imshow(xadvs_0)
        plt.imsave('/home/shayan/Proposal_Pix2Pix/Data/Training/DeepFool/Iteration_4/Retraining_Results_Pix2Pix_Adv/Epoch_10/Unfooling_Data/xadvs_' + str(i) + '_deepfool_' + '_' + str(deepfool_acc) + '_[' + str((int(y0[0])*0+int(y0[1])*1)) + '].png',xadvs_0)
        plt.close(fig)
