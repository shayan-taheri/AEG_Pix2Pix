...
def retrain_train(sess, env, X_data, y_data, X_valid=None, y_valid=None, epochs=1,
    ...
    if not hasattr(env, 'saver'):
        ...
        return
    print('\nLoading saved model')
    env.saver.restore(sess, 'iter3_retrain/adv_pix2pix/Epoch_10/{}'.format(name))
    ...
    if hasattr(env, 'saver'):
        print('\n Saving model')
        env.saver.save(sess, 'iter4_retrain/adv_pix2pix/Epoch_10/{}'.format(name))
...
def retrain_use(sess, env, X_data, y_data, X_valid=None, y_valid=None, epochs=1,
    ...
    if not hasattr(env, 'saver'):
        ...
        return
    print('\nLoading saved model')
    env.saver.restore(sess, 'iter4_retrain/adv_pix2pix/Epoch_10/{}'.format(name))
...
len_list = []
for img in glob.glob('/home/shayan/Adv_Dataset/Train/FGSM/Fooling_Data/' + '*.png'):
...
len_list = []
for img in glob.glob('/home/shayan/Proposal_Pix2Pix/Data/Training/FGSM/Iteration_4/Testing_Data/Epoch_10/outputs/' + '*.png'):
    len_list.append(len(img))
...
for i in range(0,9969):
    ...
    if fgsm_acc <= 0.20:
        fig = plt.figure()
        plt.imshow(xadvs_0)
        plt.imsave('/home/shayan/Proposal_Pix2Pix/Data/Training/FGSM/Iteration_4/Retraining_Results_Adv_Pix2Pix/Epoch_10/Fooling_Data/xadvs_' + str(i) + '_fgsm_' + '_' + str(fgsm_acc) + '_[' + str((int(y0[0])*0+int(y0[1])*1)) + '].png',xadvs_0)
        plt.close(fig)
    else:
        fig = plt.figure()
        plt.imshow(xadvs_0)
        plt.imsave('/home/shayan/Proposal_Pix2Pix/Data/Training/FGSM/Iteration_4/Retraining_Results_Adv_Pix2Pix/Epoch_10/Unfooling_Data/xadvs_' + str(i) + '_fgsm_' + '_' + str(fgsm_acc) + '_[' + str((int(y0[0])*0+int(y0[1])*1)) + '].png',xadvs_0)
        plt.close(fig)
